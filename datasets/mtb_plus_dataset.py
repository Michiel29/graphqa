...
class MTBPlusDataset(FairseqDataset):
    ...
    def sample_share_two(self, headA, tailA, textA):

        # Get edges with headA as the head
        headA_edges = self.graph_B.edges[headA].numpy().reshape(-1, GraphDataset.EDGE_SIZE)

        # Get all indices of headA's neighbors, for which the neighbor is tailA
        headA_neighbors_idxs = np.flatnonzero(headA_edges[:, GraphDataset.TAIL_ENTITY] == tailA)
        ...
        # Get all edges between headA and tailA
        headA_tailA_edges = headA_edges[headA_neighbors_idxs, :]

        # Shuffle headA_tailA_edges
        headA_tailA_edges = headA_tailA_edges[torch.randperm(len(headA_tailA_edges)).numpy()]

        # Sample textB_pos from headA_tailA_edges edges
        textB_pos = self.sample_text(headA_tailA_edges, textA, 'share_two')
    ...
    def sample_share_none(self, headA, tailA, textA, fixB_pos, newB_pos):
        ...
        return textB_strong_neg, newB_pos, newB_strong_neg
    ...
    def __getitem__(self, index):
        ...
        with data_utils.numpy_seed(9031935, self.seed, self.epoch, index):
            ...
            if task_choice == 'mtb':
                # textA and textB_pos share both headA and tailA
        ...
        if self.use_strong_negs:
            item['fixB_strong_neg'] = fixB_strong_neg
    ...
    def collater(self, instances):
        ...
        if n_textB_init == 1: # not using strong negatives
        ...
        else: # using strong negatives
            fixB_list = np.array(list(chain.from_iterable([[instance['fixB_pos'], instance['fixB_strong_neg']] for instance in instances])))
